...
void update_pong(struct Pong* pong, double dt)
{
    if (pong->state == PLAY)
    {
        ...
        // Movimiento del jugador uno (IA)
        if (pong->ball.x < TABLE_WIDTH / 2)  // Solo se mueve si la pelota está en su lado
        {
            float target_y1 = pong->ball.y - (PADDLE_HEIGHT / 2);
            pong->player1.y += (target_y1 - pong->player1.y) * 20.0 * dt; 
            if (pong->player1.y < 0) pong->player1.y = 0;
            if (pong->player1.y + PADDLE_HEIGHT > TABLE_HEIGHT) pong->player1.y = TABLE_HEIGHT - PADDLE_HEIGHT;
        }


        // Movimiento del jugador dos
        if (pong->ball.x > TABLE_WIDTH / 2)  // Solo se mueve si la pelota está el lado de la cancha del jugador dos
        {
            float target_y2 = pong->ball.y - (PADDLE_HEIGHT / 2); 
            pong->player2.y += (target_y2 - pong->player2.y) * 20.0 * dt; 
            if (pong->player2.y < 0) pong->player2.y = 0;
            if (pong->player2.y + PADDLE_HEIGHT > TABLE_HEIGHT) pong->player2.y = TABLE_HEIGHT - PADDLE_HEIGHT;
        }
